...
exports.handler = async function(event, context) {
  try {
    let feed = await parser.parseURL('http://www.metoffice.gov.uk/public/data/PWSCache/WarningsRSS/Region/UK');
        ...
        if (feed.items.length == 0) {
          ...
          let items = feed.items.map(function(item) {
            if (item.title.match(/ scotland /gi)) {
              let desc = item.contentSnippet.replace(`${item.title} : `, '');
          ...
          console.info(`${feed.items.length}  MetOffice Warnings for Scotland detected...`);
